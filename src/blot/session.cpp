...
static json::object make_result(const json::value& id, json::object result) {
  json::object msg{};
  msg["jsonrpc"] = "2.0";
  msg["id"] = id;
...
static json::object make_jsonrpc_error(
...
static void send_progress(
    session* s, const json::value& request_id, std::string_view phase,
...
json::object session::handle_initialize(
...
json::object session::handle_infer(
    const json::value& id, const json::object& params) {
  token_t tok{};
  if (params.contains("token")) {
    ...
    if (cached) {
      send_progress(this, id, "infer", "cached", 0);
      return *cached;
    }
  ...
  send_progress(this, id, "infer", "running");
  auto t0 = clock_t::now();
  ...
  try {
    ...
    send_progress(this, id, "infer", "error", ms);
    json::object data{};
    data["dribble"] = e.what();
  ...
  send_progress(this, id, "infer", "done", ms);
...
json::object session::handle_grabasm(
    const json::value& id, const json::object& params) {
  ...
  if (cached) {
    send_progress(this, id, "grabasm", "cached", 0);
    return *cached;
  }

  // Phase 2: compile outside lock
  send_progress(this, id, "grabasm", "running");
  auto t0 = clock_t::now();
  ...
  try {
    ...
    auto ms = duration_ms(t0);
    send_progress(this, id, "grabasm", "error", ms);
    json::object data{};
  ...
  auto ms = duration_ms(t0);
  send_progress(this, id, "grabasm", "done", ms);
...
json::object session::handle_annotate(
    const json::value& id, const json::object& params) {
  ...
  if (params.contains("token")) {
    ...
    if (cached) {
      send_progress(this, id, "annotate", "cached", 0);
      return *cached;
    }
  } else {
  ...
  send_progress(this, id, "annotate", "running");
  auto t0 = clock_t::now();
  ...
  try {
    ...
    auto ms = duration_ms(t0);
    send_progress(this, id, "annotate", "error", ms);
    json::object data{};
    data["dribble"] = e.what();
    return make_jsonrpc_error(id, -32603, e.what(), std::move(data));
  }
  ...
  auto ms = duration_ms(t0);
  send_progress(this, id, "annotate", "done", ms);
