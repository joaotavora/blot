#!/usr/bin/env python3
"""Test JSONRPC error handling over WebSocket."""

from web_tests_common import (
    BlotServer,
    JsonRpcError,
    fixture_ccj,
    run_tests,
...
def _assert_rpc_error(ws, method, params, expected_code):
    try:
...
def test_unknown_method():
    ...
    with BlotServer(CCJ) as srv:
        ws = srv.ws_connect()
        try:
            _assert_rpc_error(ws, 'blot/no_such_method', {}, -32601)
        finally:
            ws.close()
...
def test_infer_missing_params():
    ...
    with BlotServer(CCJ) as srv:
        ws = srv.ws_connect()
        try:
            _assert_rpc_error(ws, 'blot/infer', {}, -32602)
        finally:
            ws.close()
...
def test_infer_path_traversal():
    ...
    with BlotServer(CCJ) as srv:
        ...
        try:
            _assert_rpc_error(
                ws, 'blot/infer', {'file': '../../etc/passwd'}, -32602
            )
        finally:
            ws.close()
...
def test_infer_stale_token():
    ...
    with BlotServer(CCJ) as srv:
        ws = srv.ws_connect()
        try:
            _assert_rpc_error(ws, 'blot/infer', {'token': 999999}, -32602)
        finally:
            ws.close()
...
def test_grabasm_missing_params():
    ...
    with BlotServer(CCJ) as srv:
        ws = srv.ws_connect()
        try:
            _assert_rpc_error(ws, 'blot/grab_asm', {}, -32602)
        finally:
            ws.close()
...
def test_grabasm_stale_token():
    ...
    with BlotServer(CCJ) as srv:
        ws = srv.ws_connect()
        try:
            _assert_rpc_error(ws, 'blot/grab_asm', {'token': 999999}, -32602)
        finally:
            ws.close()
...
def test_annotate_missing_params():
    ...
    with BlotServer(CCJ) as srv:
        ws = srv.ws_connect()
        try:
            _assert_rpc_error(ws, 'blot/annotate', {}, -32602)
        finally:
            ws.close()
...
def test_annotate_stale_token():
    ...
    with BlotServer(CCJ) as srv:
        ws = srv.ws_connect()
        try:
            _assert_rpc_error(ws, 'blot/annotate', {'token': 999999}, -32602)
        finally:
            ws.close()
