...
#
# Instead of this clever script, we could require CMake 3.24 which
...
if grep -q '^compiler=apple-clang' "${profile}.1"; then
  ...
  sed 's/^\(compiler\.version=\).\{1,\}$/\111/' "${profile}.1" > "$profile"
fi

conan remove -c * #  ensure local cache is pristine
conan install . --settings build_type=Release --build=never || conan graph explain . --settings build_type=Release
